...
import { User, Menu, X, PlusCircle, Settings, LogOut, Edit3, Dog, Cat, Bird, Activity, Trash2, Check, ChevronsUpDown } from "lucide-react";
...
export default function UserProfile() {
  ...
  const [motherRequestSent, setMotherRequestSent] = useState(false);
  const [fatherRequestSent, setFatherRequestSent] = useState(false);
  const [isEditingPet, setIsEditingPet] = useState(false);
  const [editForm, setEditForm] = useState<Partial<Pet>>({});

  const [isEditProfileModalOpen, setIsEditProfileModalOpen] = useState(false);
  // NEW: Added bio and phone to the state
  const [editProfileForm, setEditProfileForm] = useState({ country: "", state: "", bio: "", phone: "" });
  const [isSavingProfile, setIsSavingProfile] = useState(false);
  const [openCountry, setOpenCountry] = useState(false);
  const [openState, setOpenState] = useState(false);

  const countries = Country.getAllCountries();
  const states = editProfileForm.country ? State.getStatesOfCountry(editProfileForm.country) : [];
  ...
  const openEditProfile = () => {
    ...
    if (dbUser?.country) {
      const matchedC = Country.getAllCountries().find(c => c.name === dbUser.country);
      if (matchedC) matchedCountryCode = matchedC.isoCode;
    }

    if (dbUser?.state && matchedCountryCode) {
      const matchedS = State.getStatesOfCountry(matchedCountryCode).find(s => s.name === dbUser.state);
      if (matchedS) matchedStateCode = matchedS.isoCode;
    }

    // NEW: Load existing bio and phone from database when opening modal
    setEditProfileForm({
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      const countryName = editProfileForm.country
        ...
        : null;
      const stateName = editProfileForm.state && editProfileForm.country
        ? State.getStateByCodeAndCountry(editProfileForm.state, editProfileForm.country)?.name
        : null;

      // NEW: Added bio and phone to the database update payload
  ...
  const openPetDetails = (pet: any) => {
    ...
    setMotherId("");
    setFatherId("");
    setMotherRequestSent(false);
    setFatherRequestSent(false);
  ...
  const handleUpdatePet = async () => {
    ...
    try {
      ...
      setIsEditingPet(false);
      setSelectedPet({ ...selectedPet, ...editForm } as Pet);
      queryClient.invalidateQueries({ queryKey: ['pets', user?.id] });
    } catch (error: any) {
      toast({ description: error.message || "Failed to update pet", variant: "destructive" });
    }
  };
  ...
  const handleDeletePet = async () => {
    ...
    if (window.confirm("Are you sure you want to delete this pet? This action cannot be undone.")) {
      try {
        ...
        toast({ description: "Pet deleted successfully!" });
        setIsPetDetailsModalOpen(false);
        queryClient.invalidateQueries({ queryKey: ['pets', user?.id] });
      } catch (error: any) {
        toast({ description: error.message || "Failed to delete pet", variant: "destructive" });
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
            ...
            {isLoading ? (
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56 mt-2">
                  ...
                  <DropdownMenuItem onClick={handleAuthClick} className="text-destructive cursor-pointer flex items-center font-medium">
                    Log Out
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
        ...
        {isMenuOpen && (
          <div className="md:hidden border-t p-4 space-y-4 bg-background animate-in slide-in-from-top-5 shadow-2xl absolute w-full z-50">
            ...
            {isLoading ? (
              ...
              <>
                <Link href="/user-profile" className="block text-base font-semibold py-3 px-4 rounded-lg bg-muted text-primary cursor-pointer">Profile Dashboard</Link>
                <Button className="w-full mt-4 cursor-pointer rounded-full py-6 text-lg" variant="destructive" onClick={handleAuthClick}>
                  Log Out
                </Button>
              </>
            ) : (
        ...
        <div className="flex flex-col md:flex-row gap-8">
              ...
              <CardContent className="pt-0 relative px-6 pb-6">
                <div className="flex justify-center -mt-12 mb-4">
                  <div className="h-24 w-24 rounded-full border-4 border-background bg-muted flex items-center justify-center overflow-hidden shadow-md">
                    {user?.id ? (
                      <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.id}`} alt="User Avatar" />
                    ) : (
                ...
                <div className="space-y-3">
                  {/* NEW: Added link to view their own public profile */}
                  <Link href={`/profile/${user?.username}`}>
                    <Button variant="outline" className="w-full justify-start cursor-pointer hover:bg-primary/5 hover:text-primary transition-colors border-[#007699] text-[#007699] mb-3">
                      <User className="mr-2 h-4 w-4" /> View Public Profile
                    </Button>
                  </Link>

                  <Button onClick={openEditProfile} variant="outline" className="w-full justify-start cursor-pointer hover:bg-primary/5 hover:text-primary transition-colors">
                    <Edit3 className="mr-2 h-4 w-4" /> Edit Profile
                  </Button>
                  <Button variant="outline" className="w-full justify-start cursor-pointer hover:bg-primary/5 hover:text-primary transition-colors">
                    <Settings className="mr-2 h-4 w-4" /> Account Settings
                  </Button>
                  {isAuthenticated && (
                    <Button variant="outline" className="w-full justify-start text-destructive hover:bg-destructive/5 hover:text-destructive cursor-pointer transition-colors" onClick={handleLogout}>
          ...
          <div className="w-full md:w-2/3 space-y-8">
            <div className="flex items-center justify-between">
              <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">My Pets</h1>
              <Button
            ...
            {isPetsLoading ? (
                ...
                {userPets.map((pet) => (
                  ...
                  >
                    <div className="h-24 bg-gradient-to-br from-[#007699]/10 to-[#FF6600]/10 flex items-center justify-center relative border-b border-gray-100">
                      {pet.type === 'dog' && <Dog className="h-12 w-12 text-[#007699]/20 absolute -bottom-4 -right-4" />}
                      {pet.type === 'cat' && <Cat className="h-12 w-12 text-[#007699]/20 absolute -bottom-4 -right-4" />}
                      {pet.type === 'bird' && <Bird className="h-12 w-12 text-[#007699]/20 absolute -bottom-4 -right-4" />}
                      ...
                      <div className="h-16 w-16 bg-white rounded-full border-4 border-white shadow-sm flex items-center justify-center translate-y-6">
                        {pet.image_url ? (
                          ...
                          pet.type === 'dog' ? <Dog className="h-8 w-8 text-[#007699]" /> :
                            pet.type === 'cat' ? <Cat className="h-8 w-8 text-[#007699]" /> :
                              pet.type === 'bird' ? <Bird className="h-8 w-8 text-[#007699]" /> :
                                <Activity className="h-8 w-8 text-[#007699]" />
                        )}
                      </div>
                    </div>
                    <CardContent className="pt-10 pb-4 text-center">
                      <h3 className="font-bold text-xl text-foreground capitalize">{pet.name}</h3>
              ...
              <Card className="border-dashed border-2 bg-transparent shadow-none border-gray-200">
                <CardContent className="flex flex-col items-center justify-center h-64 text-center">
                  ...
                  <h3 className="text-xl font-bold text-gray-700 mb-2">No pets added yet</h3>
      ...
      {/* Pet Details & Lineage Modal */}
      <Dialog open={isPetDetailsModalOpen} onOpenChange={setIsPetDetailsModalOpen}>
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-4">
                  <div className="h-16 w-16 bg-primary/10 rounded-full flex items-center justify-center overflow-hidden">
                    {selectedPet.image_url ? (
                      ...
                      selectedPet.type === 'dog' ? <Dog className="h-8 w-8 text-primary" /> :
                        selectedPet.type === 'cat' ? <Cat className="h-8 w-8 text-primary" /> :
                          selectedPet.type === 'bird' ? <Bird className="h-8 w-8 text-primary" /> :
                  ...
                  <div>
                    {isEditingPet ? (
                      ...
                      <h2 className="text-2xl font-bold capitalize">{selectedPet.name}</h2>
                    )}
              ...
              <div className="grid grid-cols-2 gap-4 text-sm mt-4">
                {isEditingPet ? (
                  <>
                    <div>
                      <span className="font-semibold text-muted-foreground block mb-1">Gender: </span>
                      <input
                    ...
                    <div>
                      <span className="font-semibold text-muted-foreground block mb-1">DOB: </span>
                      <input
                        type="date"
                        className="w-full bg-background border border-input rounded px-2 py-1"
                        value={editForm.dob || ""}
                    ...
                    <div className="col-span-2 flex justify-end mt-2">
                      <Button onClick={handleUpdatePet} className="w-full cursor-pointer bg-[#007699] hover:bg-[#005a75]">Save Changes</Button>
                    </div>
                  </>
                ) : (
                  <>
                    <div>
                      ...
                      <span className="capitalize">{selectedPet.gender || 'Unknown'}</span>
                    </div>
                    <div>
              ...
              <div className="border-t pt-4">
                <h3 className="font-bold text-lg mb-3">Link Parents</h3>
                <div className="space-y-3">
                  <div className="flex gap-2 items-end">
                    <div className="flex-1">
                      <label className="text-sm font-medium mb-1 block text-muted-foreground">Mother's Pet ID</label>
                      <input
                        type="text"
                        ...
                        disabled={motherRequestSent}
                      />
                    </div>
                    <Button
                      onClick={() => {
                        if (!motherId) return;
                        setMotherRequestSent(true);
                        toast({ description: "Request sent successfully!" });
                      }}
                      disabled={!motherId || motherRequestSent}
                      className="cursor-pointer bg-[#007699] hover:bg-[#005a75]"
                      variant={motherRequestSent ? "outline" : "default"}
                    >
                      {motherRequestSent ? <><Check className="mr-2 h-4 w-4" /> Request Sent</> : "Send"}
                    </Button>
                  </div>
                  <div className="flex gap-2 items-end">
                    <div className="flex-1">
                      <label className="text-sm font-medium mb-1 block text-muted-foreground">Father's Pet ID</label>
                      <input
                        type="text"
                        className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                        ...
                        disabled={fatherRequestSent}
                      />
                    </div>
                    <Button
                      onClick={() => {
                        if (!fatherId) return;
                        setFatherRequestSent(true);
                        toast({ description: "Request sent successfully!" });
                      }}
                      disabled={!fatherId || fatherRequestSent}
                      className="cursor-pointer bg-[#FF6600] hover:bg-[#e65c00]"
                      variant={fatherRequestSent ? "outline" : "default"}
                    >
                      {fatherRequestSent ? <><Check className="mr-2 h-4 w-4" /> Request Sent</> : "Send"}
                    </Button>
                  </div>
                </div>
              </div>
      ...
      {/* Edit Profile Settings Modal with NEW Bio and Phone Inputs */}
      <Dialog open={isEditProfileModalOpen} onOpenChange={setIsEditProfileModalOpen}>
        <DialogContent className="sm:max-w-[450px]">
          <div>
            ...
            <p className="text-sm text-gray-500 mb-4">Update your public account information.</p>
          </div>
              ...
              <Popover open={openCountry} onOpenChange={setOpenCountry}>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className="justify-between bg-white border-gray-200 focus:ring-2 focus:ring-[#007699]/20 w-full font-normal"
                  >
                    {editProfileForm.country
                      ? countries.find((c) => c.isoCode === editProfileForm.country)?.name
                      : "Select Country"}
                          ...
                          >
                            <Check
                              className={cn(
                                ...
                                editProfileForm.country === c.isoCode ? "opacity-100" : "opacity-0"
                              )}
                            />
                            {c.name}
                          </CommandItem>
              ...
              <Popover open={openState} onOpenChange={setOpenState}>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    disabled={!editProfileForm.country || states.length === 0}
                    className="justify-between bg-white border-gray-200 focus:ring-2 focus:ring-[#007699]/20 w-full font-normal"
                  >
                        ...
                        {states.map((s) => (
                          ...
                          >
                            <Check
                              className={cn(
                                "mr-2 h-4 w-4 text-[#007699]",
                                editProfileForm.state === s.isoCode ? "opacity-100" : "opacity-0"
                              )}
                            />
                            {s.name}
                          </CommandItem>
          ...
          <div className="flex justify-end mt-6 pt-4 border-t border-gray-100">
            <Button onClick={handleUpdateProfile} disabled={isSavingProfile} className="w-full sm:w-auto cursor-pointer bg-[#007699] hover:bg-[#005a75] text-white rounded-full px-8">
